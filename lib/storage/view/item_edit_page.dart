...
class _ItemEditPageState extends ConsumerState<ItemEditPage> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<ItemEditState>(itemEditProvider, (previous, next) {
      if (next.status == ItemEditStatus.loading) {
        showInfoSnackBar('正在提交...', duration: 1);
      }
      if (next.status == ItemEditStatus.addSuccess) {
        showInfoSnackBar('物品 ${next.item?.name ?? ''} 添加成功');
        Navigator.of(context).pop(true);
      }
      if (next.status == ItemEditStatus.updateSuccess) {
        showInfoSnackBar('物品 ${next.item?.name ?? ''} 修改成功');
        Navigator.of(context).pop(true);
      }
          ...
          child: Form(
            ...
            child: Column(
              children: <Widget>[
                ...
                RoundedRaisedButton(
                  onPressed: (state.status != ItemEditStatus.loading)
                      ? () {
