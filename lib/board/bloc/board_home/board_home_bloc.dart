...
class BoardHomeBloc extends Bloc<BoardHomeEvent, BoardHomeState> {
  ...
  FutureOr<void> _onBoardHomeFetched(
    BoardHomeFetched event,
    Emitter<BoardHomeState> emit,
  ) async {
    try {
      // 如果需要刷新，则显示加载界面
      // 因为需要请求网络最好提示用户
      ...
      if (event.cache &&
        ...
        final results = await boardRepository.topics(cache: event.cache);
        emit(
          state.copyWith(
            status: BoardHomeStatus.success,
      ...
      emit(state.copyWith(status: BoardHomeStatus.failure, error: e.message));
    }
  }
}
